...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--B", type=int, default=32, help="Number of branches.")
    ...
    parser.add_argument("--out", type=Path, default=Path("results/branch_fan_in_aggregation.csv"))
...
def validate_config(primary: int, groups: list[int], N: int, block: int) -> None:
    ...
    for g in groups:
        if g > primary:
            raise ValueError(f"fusion group size {g} cannot exceed B={primary}")
        if primary % g != 0:
            raise ValueError(f"B={primary} must be divisible by group size {g}")
...
def run_correctness_check(device: torch.device, primary: int, groups: list[int], check_n: int, block: int) -> None:
    ...
    print(f"[check] running correctness check with N={check_n}, B={primary}")
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print(f"[config] kind=branch_fan_in_aggregation, N={args.N}, B={primary}, groups={args.groups}, block={args.block}, warmup={args.warmup}, repeat={args.repeat}, trials={args.trials}")
        return
    ...
    for g in args.groups:
        ...
        row.update({
            "scenario_kind": "branch_fan_in_aggregation",
